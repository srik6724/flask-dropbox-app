...
def upload_file():
    ...
    if not user or not files or len(files) == 0:
        return jsonify({"error": "Missing user or files"}), 400
    ...
    for file in files:
        ...
        try:
            ...
            uploaded_files.append((filename, dropbox_path))
        except Exception as e:
            return jsonify({"error": f"Failed to upload {filename}: {str(e)}"}), 500

    # Send notification email
    try:
        file_list = "\n".join([f"{name} → {path}" for name, path in uploaded_files])
        msg = Message(
...
"""
    ...
    return jsonify({
        ...
        "files": [name for name, _ in uploaded_files]
    }), 200
